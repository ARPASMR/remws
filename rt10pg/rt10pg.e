...
feature {NONE} -- Initialization
	...
	make
		...
		do
			...
			--login_with_connection_string ("Driver={PostgreSQL};Server=10.10.0.19;Port=5432;Database=iris_base Uid=postgres;Pwd=p0stgr3S;")
			login_with_connection_string_and_connect ("Driver={PostgreSQL};Server=10.10.0.19;Port=5432;Database=iris_base Uid=postgres;Pwd=p0stgr3S;")
			set_base
			...
			--if db_spec.database_handle_name.is_case_insensitive_equal ("odbc") then
				...
				if session_control.is_connected then
					...
					io.put_string ("Unable to connect to dsn: " + dsn)
					io.put_new_line
				end
			--else
				io.put_string ("db_spec.database_handle_name: " + db_spec.database_handle_name)
				io.put_new_line
			--end
...
feature -- Preferences
	...
	init_preferences
		...
		do
			...
			display_line ("DSN password:         " + dsn_pwd,              true)
