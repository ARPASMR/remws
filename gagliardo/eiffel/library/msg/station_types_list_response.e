...
feature -- Access
	...
	id:                  INTEGER once Result := station_types_list_response_id end
	...
	to_json: STRING
		...
		do
			...
			if is_error_response then
				...
				json_representation.append ("%"id%": " + station_types_list_response_id.out)
				json_representation.append (",%"parameters_number%": " + station_types_list_response_parnum.out)
				json_representation.append ("},")
				...
				parameters_number := station_types_list_response_parnum
				json_representation.append ("{")
				json_representation.append ("%"header%": {")
				json_representation.append ("%"id%": " + station_types_list_response_id.out)
				json_representation.append (",%"parameters_number%": " + station_types_list_response_parnum.out)
				...
				loop
					...
					json_representation.append ("{%"id%": " + types_list.i_th (i).id.out + ",%"name%": %"" + types_list.i_th (i).name + "%"}")
					i := i + 1
				end
				json_representation.append ("]")
				json_representation.append ("}")
	...
	from_json (json: STRING)
		...
		do
			...
			if json_parser.is_valid and then attached json_parser.parsed_json_value as jv then
				...
				then
					key := "types_list"
					if attached {JSON_ARRAY} j_data.item (key) as j_status_list then

						types_list.wipe_out

						from i := 1
						until i = j_status_list.count + 1
						loop
							if attached {JSON_OBJECT} j_status_list.i_th (i) as j_status
							and then attached {JSON_NUMBER} j_status.item (key1) as j_id
							and then attached {JSON_STRING} j_status.item (key2) as j_name
							then
								create type.make_from_id_and_name (j_id.integer_64_item.to_integer, j_name.item)
								types_list.extend (type)
							end
							i := i + 1
						end
					end
...
feature -- XML Callbacks
	...
	on_attribute (a_namespace: detachable READABLE_STRING_32; a_prefix: detachable READABLE_STRING_32; a_local_part: READABLE_STRING_32; a_value: READABLE_STRING_32)
		...
		do
			...
			if current_tag.is_equal ("TipoStazione") then
				if a_local_part.is_equal ("Id") then
					create a_type.make_from_id_and_name (a_value.to_integer, "")
					types_list.extend (a_type)
