note
	description: "Summary description for {STATION_TYPES_LIST_RESPONSE}."
	author: ""
	date: "$Date$"
	revision: "$Revision$"

--| ----------------------------------------------------------------------------
--| This is the message structure for the station_types_list response message,
...
--|     "outcome": a_number,
--|     "message": "a_message",
--|     "types_list": [{"id": a_number, "name": "a_name"},
--|                    {"id": a_number, "name": "a_name"},
--|                    ... ,
--|                    {"id": a_number, "name": "a_name"}]
--|   }
--| }
...
feature -- Access
	...
	id:                  INTEGER do Result := station_types_list_response_id end

	outcome:             INTEGER
	message:             STRING

	types_list:          ARRAYED_LIST [STATION_TYPE]
	...
	to_json: STRING
		...
		do
			...
			if is_error_response then
				...
				Result.append (double_quotes + json_id_tag + double_quotes + colon + space + station_types_list_response_id.out)
				...
				Result.append (comma + double_quotes + json_types_list_tag + double_quotes + colon + space + left_bracket)
				from types_list.start
				until types_list.after
				loop
					Result.append (left_brace + double_quotes + json_id_tag + double_quotes + colon + space + types_list.item.id.out + comma + double_quotes + json_name_tag +
					               double_quotes + colon + space + double_quotes + types_list.item.name + double_quotes + right_brace)
		...
		do
			...
			if parser.is_valid and then attached parser.parsed_json_value as jv then
				...
				if attached {JSON_OBJECT} jv as j_object and then attached {JSON_OBJECT} j_object.item (json_data_tag) as j_data
					and then attached {JSON_ARRAY} j_data.item (json_types_list_tag) as j_status_list then

					types_list.wipe_out

					from j_status_list.array_representation.start
					until j_status_list.array_representation.after
					loop
						if attached {JSON_OBJECT} j_status_list.array_representation.item as j_status
						and then attached {JSON_NUMBER} j_status.item (json_id_tag) as j_id
						and then attached {JSON_STRING} j_status.item (json_name_tag) as j_name
						then
							types_list.extend (create {STATION_TYPE}.make_from_id_and_name (j_id.integer_64_item.to_integer, j_name.item))
						end
						j_status_list.array_representation.forth
					end
				end
			else
...
feature -- XML Callbacks
	...
	on_attribute (a_namespace: detachable READABLE_STRING_32; a_prefix: detachable READABLE_STRING_32; a_local_part: READABLE_STRING_32; a_value: READABLE_STRING_32)
			-- Start of attribute.
		do
			if attached current_tag as l_current_tag and then l_current_tag.is_equal (it_xml_station_type) then
				if a_local_part.is_equal (it_xml_id) then
					types_list.extend (create {STATION_TYPE}.make_from_id_and_name (a_value.to_integer, ""))
				elseif a_local_part.is_equal (it_xml_name) then
					types_list.last.set_name (a_value)
				end
			end
		end
