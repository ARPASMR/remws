...
feature
	...
	run_process is
		...
		do
			...
			loop
				input.read_line
				process.stdin.put_string (input.last_string)
			end
			process.stdin.flush
			process.fd_stdin.close

			if process.stderr /= Void then
				out_file := process.stderr
			else
...
feature {NONE}
	...
	create_process is
		...
		deferred
			...
			stdout_or_stderr_not_void: (process.stdout /= Void) = (process.stderr /= Void)
		end
